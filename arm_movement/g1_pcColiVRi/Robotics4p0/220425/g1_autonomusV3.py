...
class AutonomousNavigator:
    ...
    def compute_control(self, goal_x, goal_y, current_x, current_y, current_yaw):
        ...
        vy = max(min(vy, 0.2), -0.2)                # Movimiento lateral limitado
        vyaw = max(min(vyaw, self.max_vyaw), -self.max_vyaw)

        return vx, vy, vyaw, distance, yaw_error

    def rotate_to_yaw(self, target_yaw, tolerance=0.05):
    ...
    def navigate(self, tolerance_dist=0.15, alignment_yaw=0.3):
        try:
            for i, (goal_x, goal_y, goal_yaw) in enumerate(self.targets, 1):
                ...
                while time.time() - start < max_time:
                    ...
                    if dist_err < tolerance_dist:
                        if dist_err < 0.10:
                            print("Posición alcanzada con alta precisión")
                        else:
                            print("Posición suficientemente cercana: 20cm de vector normal de diferencia")
                        break
                else:
