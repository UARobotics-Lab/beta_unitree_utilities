...
def info_controles ():
        ...
        print('  1       → Shakehand')
        print('  2       → Wavehand')
        print('  3       → Wave hand and turn around')
        print('  4       → Prueba de audio y LEDs de Unitree')
        print('  5       → Saludo militar [m]')
        print('  6       → Agarrar Caja [m]')
    ...
    try:
        while True:
            ...
            if key == 'w':
                ...
                print("Brazos en posicion inicial (ready)...")
                import subprocess
                subprocess.Popen(["python3", "init_arms.py", sys.argv[1]])
            elif key == '2':
                print("Posicionando brazos. Ubique a Unitree G1 para agarrar la caja...")
                import subprocess
                subprocess.Popen(["python3", "ubicar_agarrar.py", sys.argv[1]])
            elif key == '3':
                print("Levantando brazos. Caja tomada por Unitree G1 para desplazarla...")
                import subprocess
                subprocess.Popen(["python3", "agarrar_desplazamiento.py", sys.argv[1]])
            elif key == '4':
                print("Bajando brazos. Caja soltada por Unitree G1. Proceda a retirarlo del area de carga...")
                import subprocess
