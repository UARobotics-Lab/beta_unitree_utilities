# Ejecutar una secuencia de posiciones en el Unitree G1 y liberar los motores al final (sin golpe)
import sys
import time
import math
import json
    ...
    def Stop(self):
        if self.low_state:
            for joint in self.arm_joints:
                q_actual = self.low_state.motor_state[joint].q
                self.low_cmd.motor_cmd[joint].q = q_actual
        ...
        self.thread.StopLoop()
        self.thread.Join()
        print("Motores liberados suavemente. El control arm_sdk ha sido desactivado.")
...
def main():
    ...
    print(f"\nEjecutando {len(pasos)} pasos...\nEscriba 'fin' y luego Enter para salir anticipadamente.\n")
    ...
    for i, (nombre_paso, posiciones) in enumerate(pasos):
        ...
        user_input = input("Presione Enter para continuar al siguiente paso o escriba 'fin' para terminar: ").strip().lower()
        if user_input == 'fin':
            print("Ejecución interrumpida por el usuario.")
            break

    seq.Stop()
    print("Secuencia finalizada y brazos liberados.")
