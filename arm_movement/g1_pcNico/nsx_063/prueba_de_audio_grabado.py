...
import sys
import json
from unitree_sdk2py.g1.audio.g1_audio_api import ROBOT_API_ID_AUDIO_START_PLAY
...
if __name__ == "__main__":
    ...
    def reproducir_audio(audio_client, filename: str):
        """
        Reproduce un archivo de audio MP3 en el robot Unitree G1.
        El archivo debe estar en una ruta válida en el sistema del robot.
        """
        p = {"file": filename}
        parameter = json.dumps(p)
        code, data = audio_client._Call(ROBOT_API_ID_AUDIO_START_PLAY, parameter)
        
        if code == 0:
            print(f"Reproduciendo archivo: {filename}")
        else:
            print(f"Error al reproducir el archivo (code={code})")
    ...
    input("Presione ENTER para comenzar prueba de reproduccion de audio grabado...")
    reproducir_audio(audio_client, "prueba_repro_audio.mp3")
    print('Prueba de reproduccion de audio terminada')
