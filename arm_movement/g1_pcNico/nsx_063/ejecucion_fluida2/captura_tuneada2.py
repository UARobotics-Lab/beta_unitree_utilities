...
class ArmSequence:
    def __init__(self):
        ...
        self.arm_joints = BRAZOS_Y_CINTURA
...
def duplicar_paso(pasos, contador):
    ...
    try:
        ...
        nueva_duracion = paso_original["duracion"]
...
def ejecutar_pasos_temporalmente(pasos, net_interface):
    if not pasos:
        print("No hay pasos capturados para ejecutar.")
        return

    print("\nMODOS DE EJECUCIÓN:")
    input('Ponga el robot en modo Get Ready (L1+UP) y presione enter...')
    print("  1: Ejecutar TODOS los pasos")
    print("  2: Ejecutar un solo paso específico")
    print("  3: Ejecutar un rango de pasos")
    ...
    if modo == '1':
        indices = range(len(pasos))
    elif modo == '2':
        try:
            idx = int(input("Ingrese el número del paso a ejecutar: ")) - 1
            if not (0 <= idx < len(pasos)):
                print("Índice fuera de rango.")
                return
            indices = [idx]
        ...
        try:
            inicio = int(input("Paso inicial (número): ")) - 1
            fin = int(input("Paso final (número): "))
            if not (0 <= inicio < fin <= len(pasos)):
    ...
    paso_a_paso = False
    if modo in ('1', '3'):
        opc = input("¿Desea ejecutar paso a paso con ENTER entre pasos? [s/n]: ").strip().lower()
        paso_a_paso = (opc == 's')

    print("\nEjecutando rutina temporal...\n")
    ...
    for i in indices:
        ...
        if paso_a_paso:
            input("Presione ENTER para continuar al siguiente paso...")
...
def main():
    ...
    while True:
        ...
        print("  x: Ejecutar pasos capturados hasta el momento.")
        ...
        if modo == '1':
            ...
            ejecutar_pasos_temporalmente(pasos, sys.argv[1])
        elif modo == 'e':
