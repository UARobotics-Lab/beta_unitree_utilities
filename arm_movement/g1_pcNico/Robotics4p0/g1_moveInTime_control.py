...
FORWARD_SPEED = 0.2    # Velocidad de avance en metros por segundo (m/s)
LATERAL_SPEED = 0.3    # Velocidad de desplazamiento lateral en m/s (no usado en este script)
ROTATION_SPEED = 0.2   # Velocidad de giro en radianes por segundo (rad/s)
...
def initialize_robot(network_interface):
    ...
    print("**** R1 + X ****")
    time.sleep(10.0)   # Espera a que termine la acción
...
def main():
    ...
    try:
        ...
        for _ in range(4):  # Repetir 4 veces para completar el cuadrado
            move(client, x_vel=FORWARD_SPEED, duration=4.0)  # Avanza
